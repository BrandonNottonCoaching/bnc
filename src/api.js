...
// Free, no API key. Returns simplified foods with macros PER 100g.
// Uses the v1 search endpoint (the v2 API does not support text search).
  ...
  return products
    .map((p) => {
      ...
      return {
        ...
        per100: {
          cal: Math.round(Number(kcal) || (Number(protein) || 0) * 4 + (Number(carb) || 0) * 4 + (Number(fat) || 0) * 9),
